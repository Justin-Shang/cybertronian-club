// Finance 支出页：NL 录入 + 表格 CRUD + is_major 高亮
...
import { API, type Expense, EXPENSE_CATEGORIES, money, apiFetch } from "./shared";
...
export default function FinanceExpense() {
  ...
  const [addForm, setAddForm] = useState({ tx_date: "", amount: "", category: "生活", payee: "", note: "" });

  const queryStr = new URLSearchParams(Object.entries(filter).filter(([, v]) => v).map(([k, v]) => [k, v])).toString();
  const { data: rows } = useQuery<Expense[]>({
    queryKey: ["finance-expense", filter],
    queryFn: async () => (await fetch(API + "/expense?" + queryStr)).json(),
  });
  ...
  const nlConfirm = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["finance-expense"] }); qc.invalidateQueries({ queryKey: ["finance-overview"] }); setNlResult(null); setNlText(""); },
  });
  const addMut = useMutation({
    mutationFn: (f: typeof addForm) => apiFetch("/expense", { method: "POST", body: JSON.stringify(f) }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["finance-expense"] }); qc.invalidateQueries({ queryKey: ["finance-overview"] }); setShowAdd(false); setAddForm({ tx_date: "", amount: "", category: "生活", payee: "", note: "" }); },
  });
  const delMut = useMutation({
    mutationFn: (id: number) => apiFetch("/expense/" + id, { method: "DELETE" }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["finance-expense"] }); qc.invalidateQueries({ queryKey: ["finance-overview"] }); },
  });

  return (
    <FinanceLayout>
      <div className="p-6 space-y-4">
        <h1 className="text-xl font-bold">支出明细</h1>
        <Card className="p-4">
          <div className="flex items-center gap-2 mb-2"><Sparkles className="w-4 h-4 text-primary" /><span className="text-sm font-medium">自然语言记账</span></div>
          <div className="flex gap-2">
            <Input value={nlText} onChange={(e) => setNlText(e.target.value)} placeholder="如：今天买菜花了200" onKeyDown={(e) => { if (e.key === "Enter" && nlText) nlParse.mutate(nlText); }} />
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Input value={filter.q} onChange={(e) => setFilter({ ...filter, q: e.target.value })} placeholder="搜索备注/收款方" className="w-44" />
          <Button size="sm" onClick={() => setShowAdd(!showAdd)} className="ml-auto"><Plus className="w-4 h-4" />手动新增</Button>
        </div>
        {showAdd && (
          <Card className="p-4">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
              <Input type="date" value={addForm.tx_date} onChange={(e) => setAddForm({ ...addForm, tx_date: e.target.value })} />
              ...
              <Input value={addForm.payee} onChange={(e) => setAddForm({ ...addForm, payee: e.target.value })} placeholder="收款方" />
        ...
        <Card className="overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-muted/50"><tr><th className="text-left p-2">日期</th><th className="text-right p-2">金额</th><th className="text-left p-2">分类</th><th className="text-left p-2">收款方</th><th className="text-left p-2">备注</th><th className="p-2"></th></tr></thead>
            <tbody>
              {(rows || []).map((r) => (
                <tr key={r.id} className={"border-t border-border/50 hover:bg-muted/30 " + (r.isMajor ? "bg-red-500/5" : "")}>
                  <td className="p-2">{r.txDate}</td>
                  <td className="p-2 text-right font-medium text-red-600">{money(r.amount)}</td>
                  <td className="p-2"><Badge variant={r.isMajor ? "destructive" : "secondary"}>{r.category}</Badge>{r.isMajor && <span className="ml-1 text-xs text-red-500">大额</span>}{r.generated && <span className="ml-1 text-xs text-muted-foreground">自动</span>}</td>
                  <td className="p-2 text-muted-foreground">{r.payee || "—"}</td>
              ...
              {(rows || []).length === 0 && <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">暂无支出记录</td></tr>}
            </tbody>
          </table>
        </Card>
      </div>
